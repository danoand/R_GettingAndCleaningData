...
print("Attempting to load required package: 'plyr'.")
if ( !(library("plyr", logical.return = TRUE, quietly = TRUE, verbose = FALSE)) ) {
  stop("The 'plyr' package has not been found. Stopping execution.  Please install the 'plyr' package and rerun.")
}
...
identify_feature_list <- function(in_file) {
  ...
  scope_dataframe <- function(x) {
          ...
          if ( grepl("mean", x[2], fixed=TRUE, ignore.case = TRUE) | grepl("std", x[2], fixed=TRUE, ignore.case = TRUE) ) {

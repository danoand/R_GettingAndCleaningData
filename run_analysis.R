...
print("TIDY: Attempting to load required package: 'plyr'.")
if ( !(library("plyr", logical.return = TRUE, quietly = TRUE, verbose = FALSE)) ) {
  stop("TIDY: The 'plyr' package has not been found. Stopping execution.  Please install the 'plyr' package and rerun.")
}
...
add_subject_activity_data <- function(in_file_subject, in_file_activity, in_file_observations) {
  ...
  print(paste("TIDY: Read observation data and assigned subject and activity codes in:", in_file_observations, sep = " "))
...
convert_activity_codes_to_text <- function(in_r_object) {
  ...
  for (i in 1:length(tmp_working_vector_activity_codes)) {
    ...
    print(paste("TIDY: Translated Activity codes to textual descriptions in:", in_r_object, sep = " "))
...
identify_feature_list <- function(in_file) {
  ...
  scope_dataframe <- function(x) {
          ...
          if ( grepl("mean", x[2], ignore.case = TRUE) | grepl("std", x[2], ignore.case = TRUE) ) {
...
produce_tidy_dataset <- function(in_r_object, in_filename) {
    ...
    print("TIDY: Generated a tidy data file representing the core data...")
    print(paste("TIDY: See - ", paste("tidy_output/", output_tidy_dataset, sep=""), sep = " "))
...
produce_tidy_dataset_average <- function(in_r_object, in_filename) {
  ...
  print("TIDY: Generated a tidy data file representing the average or mean of each core data variable...")
  print(paste("TIDY: See - ", paste(output_folder, output_tidy_average_dataset, sep="/"), sep = " "))
